...
export default function Comments({ postId, onClose, isPopup = false }) {
  ...
  async function handleMuteUser(userId) {
    if (!confirm(t('comments.confirm_mute'))) return
    try {
      const res = await authFetch(`${API_URL}/api/users/${userId}/mute`, { method: 'POST' })
      if (res.ok) {
        setComments(comments.filter(c => c.user?.id !== userId))
        fetchComments(page) // Refresh to fill gaps
      }
                ...
                <div className="flex items-baseline justify-between mb-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {currentUser && c.user && (currentUser.id === c.user.id || currentUser._id === c.user.id) ? (
                      ...
                      <button 
                        onClick={() => handleMuteUser(c.user.id)}
                        className="text-gray-400 hover:text-yellow-500 transition"
                        title={t('comments.mute_user')}
                        aria-label={t('comments.mute_user')}
                      >
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
